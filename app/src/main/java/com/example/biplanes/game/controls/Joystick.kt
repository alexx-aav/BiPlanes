...
class Joystick @JvmOverloads constructor(
    ...
    defStyleAttr: Int = 0
) : View(context, attrs, defStyleAttr), Touchable, BaseControl.Drawable{
    ...
    private val control = object : BaseControl(view){
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        control.onSizeChanged(w,h,oldw,oldh)
    }

    override fun isPointInBounds(x: Float, y: Float): Boolean {
        val distance = sqrt((x - control.centerX) * (x - control.centerX) + (y - control.centerY) * (y - control.centerY))
    ...
    private fun updatePosition(x: Float, y: Float) {
        ...
        if (distance <= baseRadius) {
            ...
            handleX = control.centerX + (x - control.centerX) * ratio
            handleY = control.centerY + (y - control.centerY) * ratio
        }
    }
    ...
    private fun resetPosition() {
        handleX = control.centerX
        handleY = control.centerY
    }
    ...
    override fun draw(canvas: Canvas) {
        val centerX = control.centerX
        val centerY = control.centerY
    ...
    fun getXPercent(): Float = (handleX - control.centerX) / baseRadius
    fun getYPercent(): Float = (handleY - control.centerY) / baseRadius
